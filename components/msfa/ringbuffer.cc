...
void RingBuffer::Write(const uint8_t *bytes, int size) {
  ...
  while (remaining > 0) {
    ...
    if (space_available == 0) {
      // tab5: ESP-IDF's libc has no nanosleep; yield for one millisecond
      // via usleep so the render task on the other core can drain the buffer.
      usleep(1000);
    } else {
