  ...
  for (; i < n_samples; i += N) {
    ...
    for (int note = 0; note < max_active_notes; ++note) {
      if (active_note_[note].live) {
        // tab5: render each note into a scratch buffer so a note that has decayed
    ...
    // tab5: the resonant low-pass (ResoFilter) recomputes a 4x4 float state
    // transition per sample, which is too slow for real time on the ESP32-P4
    // and is not part of the DX7 signal path anyway (Dexed drops it too).
    // The summed voices go straight to the output stage.
